...
def physics_loss(outputs, targets, timesteps, alpha):

    V_total_targets = torch.sum(targets * 6400**2, axis=(1, 2, 3))
    V_total = 8.6 * 10**6
    correction_factor = (V_total_targets) / (timesteps/96 * V_total)
    V_total_targets_corrected = correction_factor * 50 * timesteps * 48 * 3600
...
def evaluate_model(model, test_loader, criterion, device, alpha):
    ...
    with torch.no_grad():  # No need to track gradients during evaluation
        for inputs, targets in test_loader:
            ...
            timesteps = inputs[:, 2, 0, 0]
            inputs = inputs[:, :2, :, :]
            outputs = model(inputs)
            loss = criterion(outputs, targets, timesteps, alpha)
            test_loss += loss.item()
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, save_path, alpha):
    ...
    for epoch in range(num_epochs):
        ...
        # Validation Phase
        avg_val_loss = evaluate_model(model, val_loader, criterion, device, alpha)
...
def train(
    ...
    train_losses, val_losses, best_val_loss, time = train_and_validate(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, save_path, alpha)
...
def create_sequence(series, T=5, H=1):
    ...
    X = np.zeros((seq_per_sim*num_sims, T, channels+1, height, width))
