...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, dropout=None):
        ...
        self.double_conv = nn.Sequential(
            ...
            nn.Dropout2d(p=dropout) if dropout is not None else nn.Identity(),
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, dropout=None):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, hidden_channels, out_channels, bilinear=False, dropout=None):
...
class UNet_dropout(nn.Module):
    ...
    Also renamed some variables to be more specific to our application. This class allows for the use
    of dropout and a mask.

    Arguments:
    - in_channels: int, number of input channels.
    - hidden_channels: list, number of channels in hidden layers.
    - out_channels: int, number of output channels.
    - distance: int, if None, the distance is a learnable parameter and a soft mask is used instead of a hard mask.
    - bilinear: bool, whether to use linear interpolation during upscaling or transposes convolutions.
    - dropout: float, dropout rate
    """
    def __init__(self, in_channels, hidden_channels, out_channels, distance = None, bilinear=False, dropout=None):
        ...
        self.decoder = Decoder(hidden_channels, out_channels, bilinear, dropout)
