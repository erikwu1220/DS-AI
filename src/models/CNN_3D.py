...
class CNN_3D(nn.Module):
    def __init__(self, 
        ...
        self.conv1 = nn.Conv3d(input_channels, hidden_size, kernel_size=kernel_size[0], padding=1, bias=bias)
        self.conv2 = nn.Conv3d(hidden_size, hidden_size, kernel_size=kernel_size[1], padding=1, bias=bias)
        self.conv3 = nn.Conv3d(hidden_size, hidden_size, kernel_size=kernel_size[2], padding=1, bias=bias)
        self.conv4 = nn.Conv3d(hidden_size, hidden_size, kernel_size=kernel_size[3], padding=1, bias=bias)

        # Batch normalization layers
        self.batchnorm1 = nn.BatchNorm3d(num_features=hidden_size)
        self.batchnorm2 = nn.BatchNorm3d(num_features=hidden_size)
        self.batchnorm3 = nn.BatchNorm3d(num_features=hidden_size)
        self.batchnorm4 = nn.BatchNorm3d(num_features=hidden_size)

        # Pooling has no learnable parameters, so we can just use one
        self.pool = nn.AvgPool3d(kernel_size=pool_kernel_size, stride=pool_stride)

        # Definition of nonlinearity function
        self.nonlinearity=nonlinearity

        # Last layer
        self.fc = nn.Conv3d(hidden_size, output_channels, kernel_size=(2, 3, 3), stride=(2, 1, 1), padding=(0, 1, 1), bias= bias)
...
# Contents in below are copied from train.py and utils.py, and adjusted slightly to fit this application
